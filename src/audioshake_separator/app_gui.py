...
if getattr(sys, "frozen", False):
    ...
    # When run from source, use repo root for .env and config
    SCRIPT_DIR = Path(__file__).resolve().parent.parent.parent

load_dotenv(SCRIPT_DIR / ".env")

from audioshake_separator.separator import (  # noqa: E402 (load_dotenv must run first)
    VALID_EXTENSIONS,
    AuthenticationError,
    check_api_key,
    is_valid_audio_file,
...
class ProgressQueue:
    ...
    def get_nowait(self) -> tuple[str, dict] | None:
        try:
            return cast(tuple[str, dict] | None, self._queue.get_nowait())
        except queue.Empty:
            return None
